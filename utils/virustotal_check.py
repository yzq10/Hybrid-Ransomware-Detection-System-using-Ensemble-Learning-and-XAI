...
def evaluate_virustotal_logic(vt_stats: Dict) -> Dict:
    ...
    if threat_count > clean_count:
        ...
        return {
            ...
            'ratios': {
                'malware_ratio': threat_count / total_meaningful if total_meaningful > 0 else 0.0,
                'clean_ratio': clean_count / total_meaningful if total_meaningful > 0 else 0.0,
                'coverage_ratio': 1.0,  # Always 1.0 since we only process when VT has data
                'detection_count': threat_count,
                'analyzed_engines': total_meaningful,
                'total_engines': total_meaningful
            }
        }
    else:
        # FIXED: Remove artificial 0.95 cap for more accurate confidence
    ...
    try:
        ...
        if vt_response['status'] == 'not_found':
            ...
            result.update({
                'file_hash': file_hash,
                'vt_response': vt_data,
                'security_mode': 'DIRECT',  # New mode identifier
